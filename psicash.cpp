...
string ErrorMsg(const char *msg, const char *func, int line) {
  stringbuf sbuf;
  ostream os(&sbuf);
  os << msg << " @" << func << ":" << line;
...
Result<string>
PsiCash::BuildRequestParams(const PsiCash::RequestInfo& req_info, int attempt) const {
  json headers;
  headers["User-Agent"] = kPsiCashUserAgent;

  if (req_info.include_auth_tokens) {
    string s;
  ...
  json j = {
      ...
      {"path",     "/"s + kAPIServerVersion + req_info.path},
      {"query",    req_info.query_params},
      {"headers",  headers},
  };
...
Result<HTTPResult> PsiCash::MakeHTTPRequestWithRetry(const PsiCash::RequestInfo& req_info) {
  ...
  for (int i = 0; i < max_attempts; i++) {
    ...
    auto req_params = BuildRequestParams(req_info, i + 1);
    if (!req_params) {
    ...
    try {
      ...
      return MakeError(utils::Stringer("json parse failed: ", e.what(), "; id:", e.id).c_str());
    }
...
Result<PsiCash::NewExpiringPurchaseResponse> PsiCash::NewExpiringPurchase(
  ...
  auto result = MakeHTTPRequestWithRetry(req_info);
  if (!result) {
  ...
  if (result->status == kHTTPStatusOK ||
    ...
    if (result->body.empty()) {
      return MakeError(utils::Stringer("result has no body; status: ", result->status).c_str());
    }
    ...
    try {
      ...
      if (j["Authorization"].is_string()) {
        transaction_id = j["Authorization"].get<string>();
      }
      ...
      if (j["TransactionResponse"]["Values"]["Expires"].is_string()) {
        ...
        if (!server_expiry.FromISO8601(expiry_string)) {
          return MakeError(
              ("failed to parse TransactionResponse.Values.Expires; got "s + expiry_string).c_str());
        }
      }
  ...
  if (result->status == kHTTPStatusOK) {
    ...
    if (server_expiry.IsZero()) {
      return MakeError("response did not provide valid TransactionResponse.Values.Expires");
    }
